async function createSimpleTiffModule(moduleArg = {}) {
  var moduleRtn;
  ...
  function findWasmBinary() {
    if (Module["locateFile"]) {
      return locateFile("simpletiff_multiplex.wasm");
    }
    return new URL("simpletiff_multiplex.wasm", import.meta.url).href;
  }
  ...
  var __emval_create_invoker = (argCount, argTypesPtr, kind) => {
    ...
    var functionName = `methodCaller<(${argTypes.map((t) => t.name)}) => ${
      retType.name
    }>`;
